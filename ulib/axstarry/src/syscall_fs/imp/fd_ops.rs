...
use super::ctypes;
use axlog::{debug, warn, info, error};

use axfs::api::port::{
    ConsoleWinSize, TCGETS, TIOCGPGRP, TIOCGWINSZ,
...
pub fn ioctl(fd: usize, request: usize, data: usize) -> LinuxResult<c_int> {
    debug!("ioctl <= fd: {}, request: {}", fd, request);
